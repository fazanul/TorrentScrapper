...
async def lists(c, m):
    ...
    for query in m.command[1:length]:
        querys += f"{query} "
    txt = await c.send_message(m.chat.id, f"Searching For: {querys}")
    link = f"https://www.1tamilmv.golf/index.php?/search/&q={querys}&search_and_or=and&search_in=titles&sortby=relevancy"
    driver.get(link)
    title = driver.title
